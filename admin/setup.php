...
/** @var HookManager $hookmanager */
$hookmanager->initHooks(array('mailsetup', 'globalsetup'));
...
$item = $formSetup->newItem('MAIL_MYPARAM1');
$item->fieldAttr['placeholder'] = (empty($_SERVER['HTTPS']) ? 'http://' : 'https://') . $_SERVER['HTTP_HOST'];
$item->cssClass = 'minwidth500';

// Setup conf for selection of a simple string input
$item = $formSetup->newItem('MAIL_MYPARAM2');
$item->defaultFieldValue = 'default value';
$item->fieldAttr['placeholder'] = 'A placeholder here';

// Setup conf for selection of a simple textarea input but we replace the text of field title
$item = $formSetup->newItem('MAIL_MYPARAM3');
$item->nameText = $item->getNameText().' more html text ';

// Setup conf for a selection of a Thirdparty
$item = $formSetup->newItem('MAIL_MYPARAM4');
$item->setAsThirdpartyType();

// Setup conf for a selection of a boolean
$formSetup->newItem('MAIL_MYPARAM5')->setAsYesNo();

// Setup conf for a selection of an Email template of type thirdparty
$formSetup->newItem('MAIL_MYPARAM6')->setAsEmailTemplate('thirdparty');

// Setup conf for a selection of a secured key
//$formSetup->newItem('MAIL_MYPARAM7')->setAsSecureKey();

// Setup conf for a selection of a Product
$formSetup->newItem('MAIL_MYPARAM8')->setAsProduct();
...
$formSetup->newItem('MAIL_MYPARAM9')->setAsSelect($TField);

// Setup conf for a multiselect combo list
$item = $formSetup->newItem('MAIL_MYPARAM10');
$item->setAsMultiSelect($TField);
$item->helpText = $langs->transnoentities('MAIL_MYPARAM10');

// Setup conf for a category selection
$formSetup->newItem('MAIL_CATEGORY_ID_XXX')->setAsCategory('product');

// Setup conf MAIL_MYPARAM10
$item = $formSetup->newItem('MAIL_MYPARAM10');
$item->setAsColor();
...
$item = $formSetup->newItem('MAIL_MYPARAM11')->setAsHtml();
$item->nameText = $item->getNameText().' more html text ';
$item->fieldInputOverride = '';
$item->helpText = $langs->transnoentities('HelpMessage');
$item->cssClass = 'minwidth500';

$item = $formSetup->newItem('MAIL_MYPARAM12');
$item->fieldOverride = "Value forced, can't be modified";
$item->cssClass = 'minwidth500';

//$item = $formSetup->newItem('MAIL_MYPARAM13')->setAsDate();	// Not yet implemented
...
if ($action == 'updateMask') {
	...
	if ($className !== '') {
		...
		if ($module->write_file($tmpobject, $langs) > 0) {
			header("Location: ".DOL_URL_ROOT."/document.php?modulepart=mail-".strtolower($tmpobjectkey)."&file=SPECIMEN.pdf");
			return;
		} else {
	...
	if ($ret > 0) {
		if (!empty($tmpobjectkey)) {
			$constforval = 'MAIL_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
llxHeader('', $langs->trans($title), $help_url, '', 0, 0, '', '', '', 'mod-mail page-admin');
...
print dol_get_fiche_head($head, 'settings', $langs->trans($title), -1, "mail@mail");
							...
							if (preg_match('/\.modules\.php$/i', $file) && preg_match('/^(pdf_|doc_)/', $file)) {
								if (file_exists($dir.'/'.$file)) {
									...
									if ($modulequalified) {
										...
										// Default
										print '<td class="center">';
										$constforvar = 'MAIL_'.strtoupper($myTmpObjectKey).'_ADDON_PDF';
